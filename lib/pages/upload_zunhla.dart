...
class UploadSongZn extends StatefulWidget {
  const UploadSongZn({Key? key}) : super(key: key);

  @override
  State<UploadSongZn> createState() => _UploadSongPtState();
}

class _UploadSongPtState extends State<UploadSongZn> {
  late String title;

  late String hlaPhuahTu;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: title.isEmpty?Container():Text(title,style: const TextStyle(color: Colors.black,fontWeight: FontWeight.w400),),
        centerTitle: true,
        actions: [
          TextButton(onPressed: ( )async{
            ...
            if(title.isEmpty){
              ...
              try{
                ...
                if(b!=null){
                  await Services().postToDatabase('https://api.airtable.com/v0/appDWbrckcWGfl3YN/Table%202', 'key5sIyVikcL1LG8S', title, hlaPhuahTu, hlaSatu, v1, v2, v3, v4, v5, chorus, ending);
                  if (!mounted) return;
                  Navigator.pushReplacement(context,MaterialPageRoute(builder: (context)=>const UploadSongZn()));
                  Fluttertoast.showToast(
                      msg: "Upload success",
                      toastLength: Toast.LENGTH_SHORT,
                      gravity: ToastGravity.BOTTOM,
